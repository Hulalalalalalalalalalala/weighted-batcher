...
def _open_locked(path, create):
    ...
    try:
        ...
        # A failed lock must leave nothing written and no descriptor held.
        os.close(fd)
        raise
...
def append_metrics(path, line):
    ...
    Raises:
        OSError: ``line`` is not a string, the target cannot be written,
            or locking the log fails.  Nothing is written when locking
            fails.
...
def _iter_lines(path, chunksize=_READ_CHUNK):
    ...
    Reading happens one chunk at a time, so an arbitrarily long single
    line is streamed rather than requiring the whole file in memory.  The
    final unterminated tail is a possible torn write and never produced;
    callers therefore cannot mistake it for a complete line.
    """
...
def iter_metrics(path):
    ...
    read line by line, so the segment set is never loaded into memory.

    A torn unterminated tail at the end of any file is silently
    discarded; a segment holding only a torn tail yields nothing.  Blank
    empty lines (``\\n``) and CRLF blank lines (``\\r\\n``) are skipped.
    Any other complete line that is not a metrics JSON object raises
...
def recover_metrics(path):
    ...
    records, while :func:`iter_metrics` streams them.
