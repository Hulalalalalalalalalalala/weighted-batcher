...
def advance_group_metrics(path, group, token, position):
    ...
    lease has not expired and ``position`` moves the group forward: a
    forged or superseded token, an expired lease, or a position another
    member already committed past (or to) raises :class:`ValueError`,
    so one position is ever advanced at a time and no record is consumed
    twice or skipped.
