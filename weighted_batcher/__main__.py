...
def _audit(path):
    ...
    # count and chain-tail checksum as one compact JSON line.
    registered, tail = audit_metrics(path)
    sys.stdout.write(
        json.dumps([registered, tail], separators=(",", ":")) + "\n"
    )
    return 0


def _verify(path):
    # Re-check every record against the audit chain; a mismatch raises
    # and is reported as a failure with status 1 by _dispatch.
    verify_metrics(path)
    return 0
