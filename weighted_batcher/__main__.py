"""Module entry point.
    ...
    python3 -m weighted_batcher checkpoint FILE HANDLE POSITION CURSOR_FILE
...
def _checkpoint(path, handle, position, cursor_path):
    # Persist the cursor (position, handle, live record boundary); a
    # position that does not parse as an integer is a usage error.
    checkpoint_metrics(path, handle, position, cursor_path)
    return 0


def _cursor_resume(path, cursor_path):
    # Stream the records the persisted cursor points at, advancing the
    # cursor file as each record is consumed.
...
def main(argv=None):
    ...
    if args and args[0] == "checkpoint":
        ...
        try:
            ...
            return 2
        return _dispatch(
            lambda path: _checkpoint(path, args[2], position, args[4]),
            args[1],
        )
